...
//        }
//        return taskService.countChildNodes(query.getParent().link().getReferenceTask().getId());
//    }
//
//
//    @Override
//    public boolean hasChildren(TaskEntry taskEntry) {
//        return taskService.countChildNodes(taskEntry.link().getReferenceTask().getId()) > 0;
//    }
//
//    @Override
...
//        Stream<TaskLink> nodeStream;
//        if (parent == null) nodeStream = (rootTask == null) ?
//                taskService.getOrphanNodes().stream() :
//                taskService.getChildNodes(rootTask.getId()).stream();
//        else nodeStream = taskService.getChildNodes(parent.link().getReferenceTask().getId()).stream();
//        return nodeStream
//                .skip(offset)
//                .limit(limit)
//                .map(link -> new TaskEntry(link, parent));
//    }
//
//    @Override
