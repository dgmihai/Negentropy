...
//        UI.getCurrent().addPollListener(event -> updateTimer());
//        UI.getCurrent().setPollInterval(1000);
//        activeSession = routineService.initiateRoutine(node.getId());
//    }
//
...
//        if (isPaused) {
//            routineService.suspendRoutine(taskId);
//        } else {
...
//        routineService.completeRoutine(activeSession.getId());
//        TaskNode node = activeSession.getNode().getNext();
//        if (node != null) {
//            routineService.initiateRoutine(node.getId());
